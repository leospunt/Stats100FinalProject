list_of_courses_data <- split(golf_weather_data, golf_weather_data$round)
...
ggplot(model_metrics_df, aes(x = row.names(model_metrics_df), y = R_Squared)) +
  ...
  labs(x = "Round", y = "R-squared", title = "Comparative Explanatory Power of Weather Conditions by Round")
